          ...
          <el-col :span="12">
            <el-form-item label="角色" prop="roleId">
              <el-select v-model="addForm.roleId" clearable placeholder="请选择角色" style="width:100%">
                <el-option v-for="(item,$index) in roles" :key="$index" :label="item.rName" :value="item.id"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
...
export default {
  ...
  created () {
    ...
    this.$postByForm(api.host + '/comm/queryRoles').then(data => {
      this.roles = data.resultData
    })
    
    this.getData(1)
    let roleElement = JSON.parse(sessionStorage.roleElement)
    if (roleElement.employee) {
      for (let i in roleElement.employee) {
        if (roleElement.employee[i].servlet == 'addOrUpdateEmployees') {
          this.btnRole.addOrUpdate = true
        }
        if (roleElement.employee[i].servlet == 'delEmployees') {
