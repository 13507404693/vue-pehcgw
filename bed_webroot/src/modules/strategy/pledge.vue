  ...
  <div>
    <div class="page-header">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>押金策略</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
      ...
      <div class="el-tabs--border-card">
        <div class="el-tabs__content">
          ...
          <el-table :data="dataList.objList" stripe style="width: 100%" v-loading="loading">
            ...
            <el-table-column prop="deposit" label="押金金额(元)" min-width="80"></el-table-column>
    ...
    <el-dialog title="添加" :visible.sync="addDialogVisible" width="30%">
      <el-form :model="addForm" status-icon :rules="addRules" ref="addForm" label-width="100px">
        ...
        <el-form-item label="押金金额" prop="deposit">
          <el-input v-model="addForm.deposit" clearable placeholder="请输入押金金额"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="修改" :visible.sync="modifyDialogVisible" width="30%">
      <el-form :model="modifyForm" status-icon :rules="addRules" ref="modifyForm" label-width="100px">
        ...
        <el-form-item label="押金金额" prop="deposit">
          <el-input v-model="modifyForm.deposit" clearable placeholder="请输入押金金额"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  created () {
    ...
    if (roleElement.pledge) {
      console.log(roleElement.pledge)
      for (let i in roleElement.pledge) {
        if (roleElement.pledge[i].servlet == 'addOrUpdateDepositStrategy') {
          this.btnRole.addOrUpdate = true
        }
        if (roleElement.pledge[i].servlet == 'delDepositStrategy') {
  ...
  methods: {
    ...
    getData (val) {
      this.page = val
      this.loading = true
      this.$postByForm(api.host + '/strategy/queryDepositStrategy', {
        pageNum: this.page,
        pageSize: this.rows,
    ...
    submitDel (data) {
      this.$confirm('确认删除？').then(_ => {
        this.$postByForm(api.host + '/strategy/delDepositStrategy', {id: data.id}).then(data => {
    ...
    submitAdd (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$postByForm(api.host + '/strategy/addOrUpdateDepositStrategy', this.addForm).then(data => {
            if(data.resultCode==200){
              this.getData(this.page)
              this.$refs[formName].resetFields()
    ...
    submitModify (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$postByForm(api.host + '/strategy/addOrUpdateDepositStrategy', this.modifyForm).then(data => {
            if(data.resultCode==200){
              this.getData(this.page)
              this.$refs[formName].resetFields()
              this.$message.success('保存成功')
    ...
    showBind (data) {
      this.$postByForm(api.host + '/comm/selectDepositHospital', {strategyId: data.id}).then(res => {
    ...
    submitBind () {
      this.$postByForm(api.host + '/strategy/addDepositHospital', {
