  ...
  <div>
    <div class="page-header">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>消毒策略</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
      ...
      <div class="el-tabs--border-card">
        <div class="el-tabs__content">
          ...
          <el-table :data="dataList.objList" stripe style="width: 100%" v-loading="loading">
            ...
            <el-table-column prop="minNum" label="时长(分钟)" min-width="80"></el-table-column>
    ...
    <el-dialog title="添加" :visible.sync="addDialogVisible" width="30%">
      <el-form :model="addForm" status-icon :rules="addRules" ref="addForm" label-width="100px">
        ...
        <el-form-item label="开始时间" prop="startTime">
          <el-time-picker v-model="addForm.startTime" placeholder="请选择开始时间" style="width: 100%" :picker-options="{ selectableRange: '00:00:00 - 23:59:59'}" value-format="HH:mm:ss"></el-time-picker>
        </el-form-item>
        <el-form-item label="时长" prop="minNum">
          <el-input v-model="addForm.minNum" clearable placeholder="请输入时长"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="修改" :visible.sync="modifyDialogVisible" width="30%">
      <el-form :model="modifyForm" status-icon :rules="addRules" ref="modifyForm" label-width="100px">
        ...
        <el-form-item label="开始时间" prop="startTime">
          <el-time-picker v-model="modifyForm.startTime" placeholder="请选择开始时间" style="width: 100%" :picker-options="{ selectableRange: '00:00:00 - 23:59:59'}" value-format="HH:mm:ss"></el-time-picker>
        </el-form-item>
        <el-form-item label="时长" prop="minNum">
          <el-input v-model="modifyForm.minNum" clearable placeholder="请输入时长"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  created () {
    ...
    if (roleElement.disinfection) {
      console.log(roleElement.disinfection)
      for (let i in roleElement.disinfection) {
        if (roleElement.disinfection[i].servlet == 'addOrUpdateDisinfectionStrategy') {
          this.btnRole.addOrUpdate = true
        }
        if (roleElement.disinfection[i].servlet == 'delDisinfectionStrategy') {
  ...
  methods: {
    ...
    getData (val) {
      this.page = val
      this.loading = true
      this.$postByForm(api.host + '/strategy/queryDisinfectionStrategy', {
        pageNum: this.page,
        pageSize: this.rows,
    ...
    submitDel (data) {
      this.$confirm('确认删除？').then(_ => {
        this.$postByForm(api.host + '/strategy/delDisinfectionStrategy', {id: data.id}).then(data => {
    ...
    submitAdd (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$postByForm(api.host + '/strategy/addOrUpdateDisinfectionStrategy', this.addForm).then(data => {
            if (data.resultCode == 200) {
              this.getData(this.page)
              this.$refs[formName].resetFields()
    ...
    submitModify (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$postByForm(api.host + '/strategy/addOrUpdateDisinfectionStrategy', this.modifyForm).then(data => {
            if (data.resultCode == 200) {
              this.getData(this.page)
              this.$refs[formName].resetFields()
              this.$message.success('保存成功')
    ...
    showBind (data) {
      this.$postByForm(api.host + '/comm/selectDisinfectionHospital', {strategyId: data.id}).then(res => {
    ...
    submitBind () {
      this.$postByForm(api.host + '/strategy/addDisinfectionHospital', {
